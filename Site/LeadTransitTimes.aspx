<%@ Page Title="" Language="C#" MasterPageFile="~/Site/MasterPage.Master" AutoEventWireup="true" CodeBehind="LeadTransitTimes.aspx.cs" Inherits="AISReports.Site.LeadTransitTimes" %>
<%@ Register Src="~/UserControls/SearchFilter.ascx" TagName="SearchFilter" TagPrefix="uc1" %>
<%@ MasterType VirtualPath="MasterPage.master" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <uc1:SearchFilter ID="SearchFilter1" runat="server"  />
    <asp:Literal runat="server" ID="litScript" EnableViewState="false">
    </asp:Literal>
     <div style="padding-left:0px;">
        <div id="header">
            <asp:PlaceHolder runat="server" ID="phContentHeaderColumn"></asp:PlaceHolder>
        </div>
        <div id="contentwrapper">
            <div id="contentcolumn">
                <div class="innerwrapper">
                    <asp:PlaceHolder runat="server" ID="phContentCenterColumn">
                        <div style="background-color:#fff;text-align:center;padding:5px;">
                            <img src="../images/avail-demo/lead-and-transit-times.png" />
                        </div>
                    </asp:PlaceHolder>
                </div>
            </div>
        </div>
        <div id="leftcolumn">
            <div class="innerwrapper">
                <asp:PlaceHolder runat="server" ID="phContentLeftColumn"></asp:PlaceHolder>
            </div>
        </div>

        <div id="rightcolumn">
            <div class="innerwrapper">
                <asp:PlaceHolder runat="server" ID="phContentRightColumn"></asp:PlaceHolder>
            </div>
        </div>
        <div id="footer">
                <div class="innerwrapper">
                <asp:PlaceHolder runat="server" ID="phContentFooterColumn"></asp:PlaceHolder>
                </div>
        </div>
     </div>
</asp:Content>

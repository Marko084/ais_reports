...
//        }
        ...
        if (gridType == "display") {
                ...
                "fnRowCallback": function (nRow, aData, iDisplayIndex) {
                    ...
                    if (aData[1] == 0) {
                        if (pKeyFieldName == undefined) {
                            ...
                            colIndex = oBlogGrid.fnGetColumnIndex(pKeyFieldName);
                ...
                "fnRowCallback": function (nRow, aData, iDisplayIndex) {
                    ...
                    if (typeof detailLink != 'undefined') {
                        ...
                        if (typeof pKeyFieldName == 'undefined') {
                            ...
                            var colIndex = oSearchGrid.fnGetColumnIndex(pKeyFieldName);
            ...
            oEditGrid = $(table).dataTable({
                ...
                "fnRowCallback": function (nRow, aData, iDisplayIndex) {
                    var colIndex = oEditGrid.fnGetColumnIndex(pKeyFieldName);
                    var claimsStatusIdx = oEditGrid.fnGetColumnIndex("ClaimStatus");

                    $('td:eq(0)', nRow).html("<a href='#' rel='" + aData[colIndex] + "' onclick='javascript:RunQuery(this);ShowEditDialog(this); return false;'>Edit</a>&nbsp;<a href='#' rel='" + aData[colIndex] + "' onclick='javascript: RunQuery(this); return false;'>Delete</a>");

                    if (aData[claimsStatusIdx] == "P") {
                        $("td:eq(1)", nRow).html("<span class='orange-btn' rel='" + aData[claimsStatusIdx] + "'>" + aData[claimsStatusIdx] + "</span>");
                    }
                    else if (aData[claimsStatusIdx] == "O") {
                        $("td:eq(1)", nRow).html("<span class='green-btn' rel='" + aData[claimsStatusIdx] + "'>" + aData[claimsStatusIdx] + "</span>");
                    }
                    else if (aData[claimsStatusIdx] == "C") {
                        $("td:eq(1)", nRow).html("<span class='red-btn' rel='" + aData[claimsStatusIdx] + "'>" + aData[claimsStatusIdx] + "</span>");
                    }
                    else {
                        $("td:eq(1)", nRow).html("<span rel='" + aData[claimsStatusIdx] + "'>" + aData[claimsStatusIdx] + "</span>");
                    }
                 }
            });
        }

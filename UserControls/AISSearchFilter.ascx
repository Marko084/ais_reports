<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AISSearchFilter.ascx.cs" Inherits="AISReports.UserControls.AISSearchFilter" %>
<style type="text/css">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        if ((cid == "10064" || cid == "10003") && document.location.href.toLowerCase().indexOf("ncdclaimsreport")>-1) {
            $("#<% =btnPostBackSearch.ClientID %>").attr("use-postback-search", "true");
            arsClaimReportTF = true;
        }

        if ($("#<% =btnPostBackSearch.ClientID %>").attr("use-postback-search") == "true") {
            $("#btnSearch").hide();
            $("#<% =btnPostBackSearch.ClientID %>").show();
            $("#<% =btnPostBackSearch.ClientID %>").button();
        }
        else if (document.location.href.toLowerCase().indexOf("reports.aspx") > -1)
        {
            $("#btnSearch").hide();
            $("#<% =btnPostBackSearch.ClientID %>").show();
            $("#<% =btnPostBackSearch.ClientID %>").button();
    ...
    function getResult() {
        ...
        else if (window.location.href.toLowerCase().indexOf("drivertierranking") > -1) {
            LoadDriverTierRankingReport(getQueryParameters());
        }
        else {
            ...
            $(".slide-out-div input").each(function () {
                ...
                if ($(this).attr("type") == "checkbox" || $(this).attr("type") == "text") {
                   ...
                   // else {
                    ...
                    //if (window.location.href.toLowerCase().indexOf("drivertierranking") > -1) {
                    //    if (paramName.toLowerCase().endsWith("enddate") && paramValue !== "") {
                    //        paramValue = $(this).val();
                    //    }
                    //}
...
//        }
    ...
    function buildQueryParamList() {
        try {
            ...
            alert("buildQueryParamList Error: "+e.Message);
        }
    }

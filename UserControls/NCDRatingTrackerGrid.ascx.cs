...
{
    public partial class NCDRatingTrackerGrid : NCDUserControlBase
    {
        #region Variables
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            pChart.Attributes.Add("grid-type", "rating");
        ...
        private void LoadColumns()
        {
            ...
            if (DisplayFields.Length > 0)
                GridHeaderFields = DisplayFields;
        ...
        private void LoadTables()
        {
            ...
            stringBuilder.AppendFormat("UserID in (0,{0}) and ", UserID == null ? (object)"0" : (object)UserID);
            ...
            stringBuilder.Append("and ObjectName<>'vw_Lytles_ScheduleBook' and DataStoreType not in ('CallCenter') ");
        ...
        private void LoadEditFields2()
        {
            ...
            SqlDataReader dr = (SqlDataReader)DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName,KeyFieldName", string.Format("{0},{1},{2}", (object)companyID, (object)ddlTables.SelectedValue, (object)PrimaryKeyField));
            ...
            while (dr.Read())
            {
                ...
                if (str1 != "")
                {
                    str2 = string.Format("responseListName{0}", (object)str1.Replace(",", ""));

                    if (!responses.Contains(string.Format("{0}|{1}", (object)str2, (object)str1)))
                        responses.Add(string.Format("{0}|{1}", (object)str2, (object)str1));
                ...
                child3.ID = string.Format("txt{0}", (object)dr["COLUMN_NAME"].ToString());
                ...
                phEditFields.Controls.Add((Control)child1);

                if (flag)
                {
                    litHeaderRow.Text += string.Format("<th>{0}</th>", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));

                    if (stringBuilder1.Length == 0)
                    {
                        stringBuilder1.AppendFormat("{0}", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));
                        stringBuilder2.AppendFormat("{0}", (object)dr["COLUMN_NAME"].ToString());
                    }
                    else
                    {
                        stringBuilder1.AppendFormat(",{0}", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));
            ...
            if (pChart.Attributes["grid-query"].ToLower() == "editgridquery" && companyID == "9999")
                pChart.Attributes["grid-query"] = string.Format("SELECT {0} FROM {1} {2}", (object)stringBuilder2.ToString(), (object)ddlTables.SelectedValue, (object)filterByValue);
            else
                pChart.Attributes["grid-query"] = string.Format("SELECT * FROM {0} {1}", (object)ddlTables.SelectedValue, (object)filterByValue);
        ...
        private void LoadEditFields()
        {
            ...
            SqlDataReader sqlDataReader = !(PrimaryKeyField != "") || PrimaryKeyField == null ? (SqlDataReader)DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName", string.Format("{0},{1}", (object)companyID, (object)ddlTables.SelectedValue)) : (SqlDataReader)DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName,KeyFieldName", string.Format("{0},{1},{2}", (object)companyID, (object)ddlTables.SelectedValue, (object)PrimaryKeyField));
            ...
            while (sqlDataReader.Read())
            {
                if (!Convert.ToBoolean(sqlDataReader["is_computed"]))
                {
                    ...
                    else if (sqlDataReader["DATA_TYPE"].ToString().ToLower() == "varchar" && sqlDataReader["COLUMN_NAME"].ToString().ToLower() == "status")
                    {
                        child3 = new HtmlGenericControl("select");
                    ...
                    if (Convert.ToBoolean(sqlDataReader["is_identity"]) || sqlDataReader["COLUMN_NAME"].ToString().ToLower() == "gbl")
                    {
                        child3.Attributes.Add("aria-identity-column", "true");
                        child3.Attributes.Add("data-field-name", sqlDataReader["COLUMN_NAME"].ToString());
                    }
                    ...
                    if (flag)
                    {
                        ...
                        if (stringBuilder1.Length == 0)
                        {
                            stringBuilder1.AppendFormat("{0}", (object)SplitCamelCase(sqlDataReader["COLUMN_NAME"].ToString()));
                            stringBuilder2.AppendFormat("{0}", (object)sqlDataReader["COLUMN_NAME"].ToString());
                        }
                        else
                        {
                            stringBuilder1.AppendFormat(",{0}", (object)SplitCamelCase(sqlDataReader["COLUMN_NAME"].ToString()));
            ...
            if (pChart.Attributes["grid-query"].ToLower() == "editgridquery")
                pChart.Attributes["grid-query"] = string.Format("SELECT {0} FROM {1} {2}", stringBuilder2.ToString() == "" ? (object)"*" : (object)stringBuilder2.ToString(), ddlTables.SelectedValue, filterByValue);
            else

...
{
    public partial class AISClaimsGrid : NCDUserControlBase
    {
        #region Variables
        ...
        private void LoadColumns()
        {
            ...
            foreach (string str in gridHeaderFields.Split(chArray))
            {
                ...
                {
                    stringBuilder.AppendFormat("<th>{0}</th>", (object)SplitCamelCase(str));
                    ++num;
                }
            }

            litHeaderRow.Text = stringBuilder.ToString();
            litNoData.Text = string.Format("<td colspan='{0}' class='dataTables_empty' >Processing...</td>", (object)(num + 1));
        }
        ...
        private void LoadTables()
        {
            ...
            stringBuilder.AppendFormat("SELECT * FROM [DatabaseEditObjects] WHERE DataStoreType like '{0}%' AND ", (object)str);

            if (CompanyID == "10000")
                stringBuilder.AppendFormat("CompanyID in(select CompanyID from Company where CompanyID={0}) ", (object)companyID);
            else
                stringBuilder.AppendFormat("CompanyID in(select CompanyID from Company where ParentCompanyID={0} or CompanyID={0}) ", (object)companyID);
        ...
        private void LoadClaimsApprovalFields()
        {
            ...
            SqlDataReader dr = DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName,KeyFieldName,UseAdjustorClaimsApprovalModule,UseGMClaimsApprovalModule,UserControlName,ShowDetailTF", string.Format("{0},{1},{2},{3},{4},NCDClaims.ascx,{5}", (object)companyID, (object)(str1 + "_Claims_Approval"), (object)PrimaryKeyField, (object)(useAdjustorClaimsApprovalModuleTF ? 1 : 0), (object)useGMClaimsApprovalModuleTF, (object)showSurveyDetailLinkTF));
            ...
            while (dr.Read())
            {
                ...
                if (str2 != "")
                {
                    str3 = string.Format("responseListName{0}", (object)str2.Replace(",", ""));
                    if (!responses.Contains(string.Format("{0}|{1}", (object)str3, (object)str2)))
                        responses.Add(string.Format("{0}|{1}", (object)str3, (object)str2));
                }
                ...
                if (dr["COLUMN_NAME"].ToString().ToLower() == "company" || dr["COLUMN_NAME"].ToString().ToLower().StartsWith("authorizedby") || dr["COLUMN_NAME"].ToString().ToLower() == "chargetoaccount" || dr["COLUMN_NAME"].ToString().ToLower() == "drivername")
                {
                    ...
                    if (dr["COLUMN_NAME"].ToString().ToLower() == "company")
                        sqlDataReader1 = DAL.ReturnDataReader("GetPickList", "CompanyID,UserID,FieldName,UsePickList", companyID + "," + UserID + ",ClaimsGMLocation,1");
                    else if (dr["COLUMN_NAME"].ToString().ToLower().StartsWith("authorizedby"))
                        sqlDataReader1 = DAL.ReturnDataReader("GetPickList", "CompanyID,UserID,FieldName,UsePickList", companyID + "," + UserID + ",GMName,1");
                    ...
                    if (dr["COLUMN_NAME"].ToString().ToLower() == "chargetoaccount")
                    {
                        ...
                        {
                            SqlDataReader sqlDataReader2 = DAL.ReturnDataReader("GetPickList", "CompanyID,UserID,FieldName,UsePickList", companyID + "," + UserID + "," + str6 + ",1");
                            ...
                            child7.Attributes.Add("class", string.Format("aria-charge-to-fields aria-pick-list {0}", (object)str6.ToLower()));
                            ...
                            stringBuilder2.AppendLine(string.Format("if (p=='{0}') ", (object)(str6.Trim().ToLower() + "Tags")));
                            ...
                            if (str6.Trim().ToLower() == "claimsgmlocation")
                            {
                                ...
                                child6.Controls.Add((Control)child8);
                            }
                    ...
                    else if (dr["COLUMN_NAME"].ToString().ToLower() == "drivername")
                    {
                        ...
                        child12.Attributes.Add("id", string.Format("txt{0}", (object)dr["COLUMN_NAME"].ToString()));
                        ...
                        child13.Attributes.Add("id", string.Format("txt{0}", (object)dr["COLUMN_NAME"].ToString()));
                    ...
                    phClaimsApproval.Controls.Add((Control)child4);
                }
                ...
                if (child3 == null)
                {
                    ...
                    if (pickListFieldMap != null && child15 != null && pickListFieldMap.Count(p => p.FieldName.ToLower() == dr["COLUMN_NAME"].ToString().ToLower()) > 0)
                    {
                        ...
                        child15.Attributes.Add("picklist-field-name", plfMap.MapToFieldName);
                        child15.Attributes.Add("picklist-table-type", plfMap.TableName);
                    }
                    ...
                    child15.ID = string.Format("txt{0}", (object)dr["COLUMN_NAME"].ToString());
                    ...
                    if (fgList != null)
                    {
                        child4.Attributes.Add("aria-group-name", fgList.GroupName);
                        ...
                        if (!fgList.GroupDivAddedTF)
                        {
                            ...
                            child16.InnerHtml = string.Format("<span class='accordion-text'>{0}</span><img class='data-indicator' src=''/>", (object)fgList.GroupName);
                            child1.Controls.Add((Control)child16);

                            phEditFields.Controls.Add(child1);

                            foreach (FieldGroupList fieldGroup in fieldGroupList)
                            {
                                if (fieldGroup.GroupName == fgList.GroupName)
                    ...
                    if (flag)
                    {
                        ...
                        if (stringBuilder1.Length == 0)
                            stringBuilder1.AppendFormat("{0}", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));
                        else
                            stringBuilder1.AppendFormat(",{0}", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));
                    }
                }
        ...
        private void LoadEditFields2()
        {
            ...
            SqlDataReader dr = DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName,KeyFieldName,UseAdjustorClaimsApprovalModule,UseGMClaimsApprovalModule,UserControlName,ShowDetailTF", string.Format("{0},{1},{2},{3},{4},NCDClaims.ascx,{5}", (object)companyID, (object)ddlTables.SelectedValue, (object)PrimaryKeyField, (object)(useAdjustorClaimsApprovalModuleTF ? 1 : 0), (object)useGMClaimsApprovalModuleTF, (object)showSurveyDetailLinkTF));
            ...
            while (dr.Read())
            {
                ...
                if (flag)
                {
                    ...
                    if (stringBuilder.Length == 0)
                        stringBuilder.AppendFormat("{0}", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));
                    else
                        stringBuilder.AppendFormat(",{0}", (object)SplitCamelCase(dr["COLUMN_NAME"].ToString()));
                }
            }
        ...
        private void LoadEditFields()
        {
            ...
            SqlDataReader dr = !(PrimaryKeyField != "") || PrimaryKeyField == null ? DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName", string.Format("{0},{1}", (object)companyID, (object)ddlTables.SelectedValue)) : DAL.ReturnDataReader("admin_GetTableEditFields", "CompanyID,TableName,KeyFieldName", string.Format("{0},{1},{2}", (object)companyID, (object)ddlTables.SelectedValue, (object)PrimaryKeyField));
        ...
        private void CreateList()
        {
            ...
            foreach (string response in responses)
            {
                ...
                {
                    if (num == 0)
                        stringBuilder1.AppendFormat("\"{0}\"", (object)str4);
                    else
                        stringBuilder1.AppendFormat(",\"{0}\"", (object)str4);
                    ++num;
                }
                ...
                stringBuilder2.AppendLine(string.Format("if(listName=='{0}') ", (object)str1) + " { return " + str1 + "; }");
            }

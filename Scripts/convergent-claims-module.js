...
$(document).ready(function () {
    ...
    //Lost Time module fields.
    $("#edit-dialog input[data-field-name='LastDayWorkedDate']").parent().hide();
    $("#edit-dialog input[data-field-name='LostTimeBeginDate']").parent().hide();
    $("#edit-dialog input[data-field-name='LostTimeEndDate']").parent().hide();
    $("#edit-dialog input[data-field-name='TotalDaysLost']").parent().hide();
    $("#edit-dialog input[data-field-name='RestrictedBeginDate']").parent().hide();
    $("#edit-dialog input[data-field-name='RestrictedEndDate']").parent().hide();
    $("#edit-dialog input[data-field-name='TotalDaysRestricted']").parent().hide();

    //Fields used to import RiskMaster data.
    $("#edit-dialog input[data-field-name='ImportType']").parent().hide();
    $("#edit-dialog input[data-field-name='LegacyClaimNumber']").parent().hide();
        ...
        readOnlyMode = $("#ContentPlaceHolder1_SearchFilter1_txtgridreadonly").val();
        ...
        if (readOnlyMode !== "true") {
            ...
            SetCalculateLagTime();
            SetLegalFeesCalculations();
            SetSubrogationCalculations();
    ...
    //if (uid == "555" || uid === "556") {
    //    $("#ContentPlaceHolder1_ctl00_hypAddRecord").hide();
    //}
    });
    ...
    function SetReadOnlyFields() {
        ...
        $("#edit-dialog input[data-field-name='ExpensesSubrogationAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='TotalSubrogationRecoveryAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='MedicalSubrogationRecoveryAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='ExpensesSubrogationRecoveryAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='IndemnitySubrogationRecoveryAmount']").prop("disabled", true);
        ...
        //$("#edit-dialog input[data-field-name='IndemnityImportAmount']").prop("disabled", true);

        $("#edit-dialog input[data-field-name='LegalFeesPaidAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='LegalFeesOutstandingAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='LegalFeesSubrogationRecoveryAmount']").prop("disabled", true);
        $("#edit-dialog input[data-field-name='LegalFeesIncurredTotalAmount']").prop("disabled", true);
        ...
        $("#edit-dialog input[data-field-name='UnallocatedAmount']").parent().hide();
        $("#edit-dialog input[data-field-name='ExpensesUnallocatedPaidAmount']").parent().hide();
    ...
    function SetInputMask() {
        ...
        $("#edit-dialog input").each(function () {
            ...
            else if ($(this).attr("data-field-name").toLowerCase() === "timeofincident") {
                //alert("Here!!");
                $(this).timepicker({
                    timeFormat: 'h:mm p',
                    interval: 30,
            ...
            if ($(this).attr("aria-data-type") === "numeric" && $(this).val() === "" &&
                ...
                $(this).mask('00000000.00', { reverse: true, translation: { '0': { pattern: /-|\d/, recursive: true } } });
                $(this).val("0.00");
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (modeDisplay === "get") {
                        ...
                        $.each(record, function (index, item) {
                            try {
                                ...
                                if (item.split("~")[1] !== undefined) {
                                    fieldValue = item.split("~")[1].replace("1/1/1900 12:00:00 AM", "").replace("12:00:00 AM", "");
                                }
                                ...
                                if (fieldName === "PolicyType") {
                                    ...
                                    policyType = fieldValue;
                                    $("#edit-dialog #lnkCheckRequest").attr("aria-policytype", fieldValue);
                                }

                                if (fieldName === "ConvergentClientAbbreviation") {
                                    HideFieldsByUserGroup(fieldValue);
                                }
                            }
                            catch (e) {
                                alert("RunQuery Get Error: "+e);
                                console.log(e);
                            }
                        });
                        ...
                        HideFieldsByPolicyType(policyType);
                    ...
                    if (modeDisplay === "updated" || modeDisplay === "view checks update") {
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            beforeSend: function () { },
                            success: function (data) { SetPickLists(); SetReadOnlyFields();},
                            error: function (xhr, textStatus, error) { SetPickLists(); alert("New User Query Error:"+ xhr.responseText); }
                        });
                    }
                    ...
                    $("#edit-dialog input[data-field-name='LostTimeBeginDate']").change();
                    $("#edit-dialog input[data-field-name='RestrictedBeginDate']").change();
                },
...
function ShowClaimsEditDialog(obj) {
    var actionType = $(obj).text().toLowerCase();

    if (actionType === "add claim") {
        ClearFields();
        GetFieldList("", "claim");
        $('#lnkOK').html('<span class=ui-button-text>Add</span>');
    }
    ...
    function AssignAdjuster() {
        ...
        if ($("#edit-dialog #assigned-adjuster").length>0) {
            ...
            $(adjusterCtrl).change(function () {
                $("#edit-dialog input[data-field-name='UserID']").val($(this).find("option:selected").val());
                $("#edit-dialog input[data-field-name='AdjusterName']").val($(this).find("option:selected").text());
            });
        }
        ...
        if (recordUserID !== "" && recordUserID !== undefined) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "../AISWS.asmx/NCDUsersQuery",
                ...
                dataType: "json",
                beforeSend: function () { },
                error: function (xhr, textStatus, error) { alert("Assign Adjuster Error: " + xhr.responseText); },
                success: function (data) {
                    var record = data.d.Users;
                    var userName;
                    var adjusterFoundTF = false;
                    var adjusterAssigned = $("#edit-dialog input[data-field-name='AdjusterName']").val();
                    ...
                    if (adjusterAssigned.length > 0) {
                        $("#assigned-adjuster option").filter(function () {
                            return $(this).text().toUpperCase() == adjusterAssigned.toUpperCase();
                        }).prop('selected', true);
                    }

                    $("#edit-dialog input[data-field-name='AdjusterName']").prop('disabled', true);
                }
            });
        }
...
function SetTTDCalculations() {
        ...
        var indemnityImportAmt = $("#edit-dialog input[data-field-name='IndemnityImportAmount']")
        var indemnitySubrogationRecoveryAmt = $("#edit-dialog input[data-field-name='IndemnitySubrogationRecoveryAmount']");
        ...
        var indemnityImportVal;
        var indemnitySubrogationRecoveryAmtVal = 0.00;
    ...
    $(ttdAWWAmount).change(function () {
        ...
        indemnityImportVal = accounting.unformat($(indemnityImportAmt).val());
        indemnitySubrogationRecoveryAmtVal = accounting.unformat($(indemnitySubrogationRecoveryAmt).val());
            
        $(indemnityOutstandingAmt).val(accounting.formatMoney(ttdTotalVal + ppdTotalVal + indemnityImportVal + indemnitySubrogationRecoveryAmtVal - indemnityPaidAmtVal));
        $(indemnityReserveAmt).val(accounting.formatMoney(ttdTotalVal + ppdTotalVal + indemnityImportVal));
    ...
    $(indemnityImportAmt).change(function () {
        $(this).val(accounting.formatMoney($(this).val()));
        ttdAmountVal = accounting.unformat($(ttdAmount).val());
        awwAmountVal = accounting.unformat($(this).val());
        ttdVal = accounting.unformat($(ttd).val());
    ...
    function SetPPDCalculations() {
        ...
        var indemnityPaidAmtVal;
        var indemnityImportVal = 0.00;
        var awwAmount = $("#edit-dialog input[data-field-name='AWW']");
        var ppdAWWAmount = $("#edit-dialog input[data-field-name='PPDAWWAmount']");
        var indemnityImportAmtField = $("#edit-dialog input[data-field-name='IndemnityImportAmount']").parent();
        var indemnityImportAmt = $("#edit-dialog input[data-field-name='IndemnityImportAmount']")

        indemnityImportAmtField.addClass("subtotal-calc-section");
        ...
        $(ppdAWWAmount).parent().after($(indemnityImportAmtField));
        ...
        $(ppdAWWAmount).change(function () {
            ...
            indemnityReserveAmtTotal = ttdTotalVal + ppdTotalVal+indemnityImportVal;
    ...
    function SetExpenseCalculations() {
        ...
        var buildingRepairAmount = $("#edit-dialog input[data-field-name='BuildingRepairAmount']");
        var equipmentBillAmount = $("#edit-dialog input[data-field-name='EquipmentBillAmount']");
        var otherDamageAmount = $("#edit-dialog input[data-field-name='OtherDamageAmount']");
        var rentalAmount = $("#edit-dialog input[data-field-name='RentalAmount']");
        var storageFees = $("#edit-dialog input[data-field-name='StorageFees']");
        var downTime = $("#edit-dialog input[data-field-name='DownTime']");
        var miscPDExpense = $("#edit-dialog input[data-field-name='MiscPDExpense']");
        var expensesTotal = 0.00;
        var expensesSubrogationRecoveryAmt = $("#edit-dialog input[data-field-name='ExpensesSubrogationRecoveryAmount']");
        ...
        var expensesSubrogationRecoveryAmtTotal = 0.00;
        ...
        $(estimatedPAndS).blur(function () {
            try {
                expensesTotal = accounting.unformat($(nurseCaseManagerFees).val()) +
                    ...
                    accounting.unformat($(axReconstructionFee).val()) + accounting.unformat($(lostWages).val()) +
                    accounting.unformat($(buildingRepairAmount).val()) + accounting.unformat($(equipmentBillAmount).val()) +
                    accounting.unformat($(otherDamageAmount).val()) + accounting.unformat($(rentalAmount).val()) +
                    accounting.unformat($(storageFees).val()) + accounting.unformat($(downTime).val()) + accounting.unformat($(miscPDExpense).val()) +
                ...
                expensesSubrogationRecoveryAmtTotal = accounting.unformat($(expensesSubrogationRecoveryAmt).val());
                expensesOutstandingAmtTotal = expensesTotal + expensesSubrogationRecoveryAmtTotal - expensesAllocatedAmtTotal;
        ...
        $(vocRehabCosts).blur(function () {
            try {
                expensesTotal = accounting.unformat($(nurseCaseManagerFees).val()) +
                             ...
                             accounting.unformat($(urReviews).val()) + accounting.unformat($(ediExpenses).val()) +
                             accounting.unformat($(vocRehabCosts).val()) + accounting.unformat($(witnessFees).val()) +
                    accounting.unformat($(axReconstructionFee).val()) + accounting.unformat($(lostWages).val()) + accounting.unformat($(buildingRepairAmount).val()) + accounting.unformat($(equipmentBillAmount).val()) +
                    accounting.unformat($(otherDamageAmount).val()) + accounting.unformat($(rentalAmount).val()) +
                    accounting.unformat($(storageFees).val()) + accounting.unformat($(downTime).val()) + accounting.unformat($(miscPDExpense).val()) +
                             accounting.unformat($(estimatedPAndS).val()) + accounting.unformat($(expertFees).val());
        ...
        $(nurseCaseManagerFees).blur(function () {
            try {
                ...
                expensesUnallocatedAmtTotal = accounting.unformat($(expensesUnallocatedAmt).val());
                expensesSubrogationRecoveryAmtTotal = accounting.unformat($(expensesSubrogationRecoveryAmt).val());
                expensesOutstandingAmtTotal = expensesTotal + expensesSubrogationRecoveryAmtTotal - (expensesAllocatedAmtTotal + expensesUnallocatedAmtTotal);
        ...
        $(buildingRepairAmount).blur(function () {
            try {
                expensesTotal = accounting.unformat($(nurseCaseManagerFees).val()) +
                    accounting.unformat($(imeCosts).val()) + accounting.unformat($(surveillanceCosts).val()) +
                    accounting.unformat($(outsideAdjusterExpense).val()) + accounting.unformat($(billReviewFees).val()) +
        ...
        $(equipmentBillAmount).blur(function () {
            try {
                expensesTotal = accounting.unformat($(nurseCaseManagerFees).val()) +
                    accounting.unformat($(imeCosts).val()) + accounting.unformat($(surveillanceCosts).val()) +
                    accounting.unformat($(outsideAdjusterExpense).val()) + accounting.unformat($(billReviewFees).val()) +
        ...
        $(otherDamageAmount).blur(function () {
            try {
                expensesTotal = accounting.unformat($(nurseCaseManagerFees).val()) +
                    accounting.unformat($(imeCosts).val()) + accounting.unformat($(surveillanceCosts).val()) +
                    accounting.unformat($(outsideAdjusterExpense).val()) + accounting.unformat($(billReviewFees).val()) +
    ...
    function GetExpenseCalculations() {
        ...
        expensesTotal = accounting.unformat($(nurseCaseManagerFees).val()) +
                             ...
                             accounting.unformat($(lostWages).val()) + accounting.unformat($(estimatedPAndS).val());

        return expensesTotal;
}

    function SetSubrogationCalculations() {
        var indemnityAmount = $("#edit-dialog input[data-field-name='IndemnityAmount']");
        var medicalAmount = $("#edit-dialog input[data-field-name='MedicalAmount']");
        var allocatedAmount = $("#edit-dialog input[data-field-name='AllocatedAmount']");
        var unallocatedAmount = $("#edit-dialog input[data-field-name='UnallocatedAmount']");
        var legalFeesAmount = $("#edit-dialog input[data-field-name='LegalFeesAmount']");
        var indemnitySubrogationRecoveryAmount = $("#edit-dialog input[data-field-name='IndemnitySubrogationRecoveryAmount']");
        var medicalSubrogationRecoveryAmount = $("#edit-dialog input[data-field-name='MedicalSubrogationRecoveryAmount']");
        var totalSubrogationRecoveryAmount = $("#edit-dialog input[data-field-name='TotalSubrogationRecoveryAmount']");
        var expensesSubrogationRecoveryAmount = $("#edit-dialog input[data-field-name='ExpensesSubrogationRecoveryAmount']");
        var legalFeesSubrogationRecoveryAmount = $("#edit-dialog input[data-field-name='LegalFeesSubrogationRecoveryAmount']");
        var unallocatedAmtVal = 0.00;
        var allocatedAmtVal = 0.00;
        var medicalAmtVal = 0.00;
        var indemnityAmtVal = 0.00;
        var legalFeesAmtVal = 0.00;
        var totalsubrogationAmtVal = 0.00;

        $(indemnityAmount).blur(function () {
            indemnityAmtVal = Math.abs(accounting.unformat($(this).val()));
            totalsubrogationAmtVal = accounting.unformat($(this).val()) + accounting.unformat($(medicalAmount).val()) +
                                    accounting.unformat($(allocatedAmount).val()) + accounting.unformat($(unallocatedAmount).val()) +
                                    accounting.unformat($(legalFeesAmount).val());

            $(totalSubrogationRecoveryAmount).val(accounting.formatMoney(totalsubrogationAmtVal));
            $(indemnitySubrogationRecoveryAmount).val(accounting.formatMoney(indemnityAmtVal));
        ...
        $(medicalAmount).blur(function () {
            medicalAmtVal = Math.abs(accounting.unformat($(this).val()));
            totalsubrogationAmtVal = Math.abs(accounting.unformat($(this).val()) + accounting.unformat($(indemnityAmount).val()) + accounting.unformat($(allocatedAmount).val()) + accounting.unformat($(unallocatedAmount).val()) + accounting.unformat($(legalFeesAmount).val()));

            $(totalSubrogationRecoveryAmount).val(accounting.formatMoney(totalsubrogationAmtVal));
            $(medicalSubrogationRecoveryAmount).val(accounting.formatMoney(medicalAmtVal));
        ...
        $(allocatedAmount).blur(function () {
            allocatedAmtVal = Math.abs(accounting.unformat($(this).val()) + accounting.unformat($(unallocatedAmount).val()));
            totalsubrogationAmtVal = Math.abs(accounting.unformat($(this).val()) + accounting.unformat($(indemnityAmount).val()) + accounting.unformat($(medicalAmount).val()) + accounting.unformat($(unallocatedAmount).val()) + accounting.unformat($(legalFeesAmount).val()));

            $(totalSubrogationRecoveryAmount).val(accounting.formatMoney(totalsubrogationAmtVal));
            $(expensesSubrogationRecoveryAmount).val(accounting.formatMoney(Math.abs(allocatedAmtVal)));
        ...
        $(unallocatedAmount).blur(function () {
            unallocatedAmtVal = Math.abs(accounting.unformat($(this).val()) + accounting.unformat($(allocatedAmount).val()));
            totalsubrogationAmtVal = Math.abs(accounting.unformat($(this).val()) + accounting.unformat($(indemnityAmount).val()) + accounting.unformat($(medicalAmount).val()) + accounting.unformat($(allocatedAmount).val()) + accounting.unformat($(legalFeesAmount).val()));

            $(totalSubrogationRecoveryAmount).val(accounting.formatMoney(totalsubrogationAmtVal));
            $(expensesSubrogationRecoveryAmount).val(accounting.formatMoney(unallocatedAmtVal));
        ...
        $(legalFeesAmount).blur(function () {
            legalFeesAmtVal = Math.abs(accounting.unformat($(this).val()));
            totalsubrogationAmtVal = Math.abs(accounting.unformat($(this).val()) + accounting.unformat($(indemnityAmount).val()) + accounting.unformat($(medicalAmount).val()) + accounting.unformat($(allocatedAmount).val()) + accounting.unformat($(unallocatedAmount).val()));

            $(totalSubrogationRecoveryAmount).val(accounting.formatMoney(totalsubrogationAmtVal));
            $(legalFeesSubrogationRecoveryAmount).val(accounting.formatMoney(legalFeesAmtVal));
    ...
    function SetLegalFeesCalculations() {
        var legalFees = $("#edit-dialog input[data-field-name='LegalFees']");
        var miscLegalExpensesAmount = $("#edit-dialog input[data-field-name='MiscLegalExpensesAmount']");
        var legalFeesPaidAmt = $("#edit-dialog input[data-field-name='LegalFeesPaidAmount']");
        var legalFeesOutstandingAmt = $("#edit-dialog input[data-field-name='LegalFeesOutstandingAmount']");
        var legalFeesIncurredAmtTotal = $("#edit-dialog input[data-field-name='LegalFeesIncurredTotalAmount']");
        var defenseAttorneyFees = $("#edit-dialog input[data-field-name='DefenseAttorneyFees']");
        var legalFeesSubrogationRecoveryAmt = $("#edit-dialog input[data-field-name='LegalFeesSubrogationRecoveryAmount']");
        var legalFeesTotal = 0.00;
        var legalFeesPaidAmtVal = 0.00;
        var legalFeesIncurredAmtVal = 0.00;
        var legalFeesSubrogationRecoveryAmtVal = 0.00;
 
        $(legalFees).blur(function () {
            legalFeesTotal = accounting.unformat($(this).val()) + accounting.unformat($(miscLegalExpensesAmount).val()) +
                accounting.unformat($(defenseAttorneyFees).val());

            legalFeesPaidAmtVal = accounting.unformat($(legalFeesPaidAmt).val());
            legalFeesSubrogationRecoveryAmtVal = accounting.unformat($(legalFeesSubrogationRecoveryAmt).val());

            $(legalFeesOutstandingAmt).val(accounting.formatMoney(legalFeesTotal + legalFeesSubrogationRecoveryAmtVal - legalFeesPaidAmtVal));
            $(legalFeesIncurredAmtTotal).val(accounting.formatMoney(legalFeesTotal));
            $(this).val(accounting.formatMoney($(this).val()));
        ...
        $(miscLegalExpensesAmount).blur(function () {
            legalFeesTotal = accounting.unformat($(legalFees).val()) + accounting.unformat($(this).val()) +
                accounting.unformat($(defenseAttorneyFees).val());
        ...
        $(defenseAttorneyFees).blur(function () {
            legalFeesTotal = accounting.unformat($(legalFees).val()) + accounting.unformat($(miscLegalExpensesAmount).val()) +
                             accounting.unformat($(this).val());
    ...
    function SetMedicalCalculations() {
        ...
        var medicalSubrogationRecoveryAmt = $("#edit-dialog input[data-field-name='MedicalSubrogationRecoveryAmount']");
        var medicalTotal = 0.00;
        var medicalPaidAmtVal = 0.00;
        var medicalSubrogationRecoveryAmtVal = 0.00;
        ...
        $(medical).blur(function () {
            ...
            medicalSubrogationRecoveryAmtVal = accounting.unformat($(medicalSubrogationRecoveryAmt).val());

            $(medicalOutstandingAmt).val(accounting.formatMoney(medicalTotal + medicalSubrogationRecoveryAmtVal - medicalPaidAmtVal));
    ...
    function SetTotalReserveAmounts() {
        ...
        var legalFeesPaidAmt = accounting.unformat($("#edit-dialog input[data-field-name='LegalFeesPaidAmount']").val()); 
        ...
        var legalFeesOutstandingAmt = accounting.unformat($("#edit-dialog input[data-field-name='LegalFeesOutstandingAmount']").val());
        ...
        var legalFeesIncurredTotalAmt = accounting.unformat($("#edit-dialog input[data-field-name='LegalFeesIncurredTotalAmount']").val());

        var indemnitySubrogationRecoveryAmt = Math.abs(accounting.unformat($("#edit-dialog input[data-field-name='IndemnitySubrogationRecoveryAmount']").val()));
        var expensesSubrogationRecoveryAmt = Math.abs(accounting.unformat($("#edit-dialog input[data-field-name='ExpensesSubrogationRecoveryAmount']").val()));
        var medicalSubrogationRecoveryAmt = Math.abs(accounting.unformat($("#edit-dialog input[data-field-name='MedicalSubrogationRecoveryAmount']").val()));
        var legalFeesSubrogationRecoveryAmt = Math.abs(accounting.unformat($("#edit-dialog input[data-field-name='LegalFeesSubrogationRecoveryAmount']").val()));

        //var indemnityIncurredAmtTotal = $("#edit-dialog input[data-field-name='IndemnityIncurredAmountTotal']");
        //var medicalIncurredAmtTotal = $("#edit-dialog input[data-field-name='MedicalIncurredAmountTotal']");
        //var expensesIncurredAmtTotal = $("#edit-dialog input[data-field-name='ExpensesTotalIncurredAmount']");
        //var legalFeesIncurredAmtTotal = $("#edit-dialog input[data-field-name='LegalFeesIncurredTotalAmount']");
        ...
        var totalSubrogationAmt = $("#edit-dialog input[data-field-name='TotalSubrogationAmount']");

        //$(indemnityIncurredAmtTotal).val(accounting.formatMoney(indemnityPaidAmt + indemnityOutstandingAmt));
        //$(medicalIncurredAmtTotal).val(accounting.formatMoney(medicalPaidAmt + medicalOutstandingAmt + medicalSubrogationRecoveryAmt));
        //$(expensesIncurredAmtTotal).val(accounting.formatMoney(expensesAllocatedPaidAmt + expensesUnallocatedPaidAmt + expensesOutstandingAmt + expensesSubrogationRecoveryAmt));
        //$(legalFeesIncurredAmtTotal).val(accounting.formatMoney(legalFeesPaidAmt + legalFeesOutstandingAmt + legalFeesSubrogationRecoveryAmt));

        $(totalPaidAmt).val(accounting.formatMoney(medicalPaidAmt + indemnityPaidAmt + expensesAllocatedPaidAmt + legalFeesPaidAmt));
        $(totalOutstandingAmt).val(accounting.formatMoney(indemnityOutstandingAmt + expensesOutstandingAmt + medicalOutstandingAmt + legalFeesOutstandingAmt));
        $(totalIncurredAmt).val(accounting.formatMoney(indemnityIncurredAmt + medicalIncurredAmt + expensesIncurredAmt + legalFeesIncurredTotalAmt));
        $(totalSubrogationAmt).val(accounting.formatMoney(indemnitySubrogationRecoveryAmt + medicalSubrogationRecoveryAmt + expensesSubrogationRecoveryAmt + legalFeesSubrogationRecoveryAmt));
    }

function ZeroOutOutstandingAmounts() {

    var claimClosedDate = $("#edit-dialog input[data-field-name='ClaimClosedDate']").val();
    var claimReOpenedDate = $("#edit-dialog input[data-field-name='ClaimReOpenedDate']").val();
    var claimReClosedDate = $("#edit-dialog input[data-field-name='ClaimReClosedDate']").val();
    var zeroAmt = accounting.unformat(0.00);

    if ((claimClosedDate.trim().length > 0 && claimReOpenedDate.trim().length === 0) || claimReClosedDate.trim().length > 0) {
        $("#edit-dialog input[data-field-name='ExpensesUnallocatedPaidAmount']").val(zeroAmt);
        $("#edit-dialog input[data-field-name='IndemnityOutstandingAmount']").val(zeroAmt);
        $("#edit-dialog input[data-field-name='ExpensesOutstandingAmountTotal']").val(zeroAmt);
        $("#edit-dialog input[data-field-name='MedicalOutstandingAmount']").val(zeroAmt); 
        $("#edit-dialog input[data-field-name='LegalFeesOutstandingAmount']").val(zeroAmt); 
        $("#edit-dialog input[data-field-name='TotalOutstandingAmount']").val(zeroAmt); 
    }
}

function HideFieldsByUserGroup(clientcode) {

    //var bentonList = "554|555|556|557|558|559|560|561|562";

    if (clientcode.toLowerCase().indexOf("bge")>-1) {
        $("#edit-dialog input[data-field-name='Division']").parent().show();
        $("#edit-dialog input[data-field-name='Superintendent']").parent().show();
        $("#edit-dialog input[data-field-name='Foreman']").parent().show();
        $("#edit-dialog input[data-field-name='Pictures']").parent().show();
        $("#edit-dialog input[data-field-name='IncidentReportedToCarrier']").parent().show();
        $("#edit-dialog input[data-field-name='IncidentReportReceived']").parent().show();
        $("#edit-dialog input[data-field-name='TypeOfIncident']").parent().show();
    }
    else {
        $("#edit-dialog input[data-field-name='Division']").parent().hide();
        $("#edit-dialog input[data-field-name='Superintendent']").parent().hide();
        $("#edit-dialog input[data-field-name='Foreman']").parent().hide();
        $("#edit-dialog input[data-field-name='Pictures']").parent().hide();
        $("#edit-dialog input[data-field-name='IncidentReportedToCarrier']").parent().hide();
        $("#edit-dialog input[data-field-name='IncidentReportReceived']").parent().hide();
        $("#edit-dialog input[data-field-name='TypeOfIncident']").parent().hide();
        $("#edit-dialog input[data-field-name='ClientLocationCode']").removeClass("aria-pick-list");
    }
}
    ...
    function HideFieldsByPolicyType(ptype) {
        ...
        if (typeof ptype !== 'undefined') {
            ...
            else if (ptype.toLowerCase() === "gl" || ptype.toLowerCase() === "property") {
        ...
        if (policyType.trim().toUpperCase() === "WC") {
            $("#edit-dialog input[data-field-name='Compensability']").parent().find("span").text("Compensability");
            $("#edit-dialog textarea[data-field-name='AccidentAddressLocation']").parent().find("span").text("Accident Address Location");
            $("#edit-dialog input[data-field-name='AccidentDescription']").parent().find("span").text("Accident Description");
            $("#edit-dialog textarea[data-field-name='NatureOfInjury']").parent().find("span").text("Nature Of Injury"); 

            $("#edit-dialog textarea[data-field-name='PartOfBody']").parent().show();
            ...
            $("#edit-dialog input[data-field-name='LegalFees']").parent().show();
            ...
            $("#edit-dialog input[data-field-name='IndemnityImportAmount']").parent().show();
        ...
        else if (policyType.trim().toUpperCase() === "AL") {

            $("#edit-dialog input[data-field-name='Compensability']").parent().find("span").text("Liability");
            $("#edit-dialog textarea[data-field-name='AccidentAddressLocation']").parent().find("span").text("Incident Address Location");
            $("#edit-dialog input[data-field-name='AccidentDescription']").parent().find("span").text("Incident Description");
            $("#edit-dialog textarea[data-field-name='NatureOfInjury']").parent().find("span").text("Nature Of Incident"); 
            ...
            $("#edit-dialog textarea[data-field-name='PartOfBody']").parent().hide();
            ...
            $("#edit-dialog input[data-field-name='RestrictedEndDate']").parent().hide();
            $("#edit-dialog input[data-field-name='TotalDaysRestricted']").parent().hide();
            $("#edit-dialog input[data-field-name='IndemnityImportAmount']").parent().hide();

            $("#edit-dialog .group-sub-label").each(function () {
                if ($(this).find("span").text() === "Indemnity" || $(this).find("span").text() === "Lost/Res Time") {
                    $(this).hide();
                }
                else {
                    $(this).show();
            ...
            if ($("#LostTimeModuleSection").length > 0) {
                $("#LostTimeModuleSection").hide();
                $("LostTimeSectionHeader").hide();
            }
        ...
        else if (policyType.trim().toUpperCase() === "GL" || ptype.toLowerCase() === "property") {
    ...
    function LockEditFields() {
        ...
        if (userType === "Manager" || userType === "ClientEditor" || userType === "Admin" || userType==="Adjuster") {
            $("#edit-dialog input").prop("disabled", false);
            $("#edit-dialog textarea").prop("disabled", false);
            $(".view-docs-btn").attr("add", "true");
        }
...
function LoadLostTimeModule() {
    var lostTimeSection = $('<div id="LostTimeSectionHeader" class="edit-field ui-accordion-header ui-state-active ui-corner-all group-sub-label" aria-group-name="Lost Time Module"><span class="accordion-text">Lost/Res Time</span></div><p id="LostTimeModuleSection"></p>');
    var lastDayWorkedDate = $("#edit-dialog input[data-field-name='LastDayWorkedDate']");
    var lostTimeBeginDate = $("#edit-dialog input[data-field-name='LostTimeBeginDate']");
    var lostTimeEndDate = $("#edit-dialog input[data-field-name='LostTimeEndDate']");
    var totalDaysLost = $("#edit-dialog input[data-field-name='TotalDaysLost']");
    var restrictedBeginDate = $("#edit-dialog input[data-field-name='RestrictedBeginDate']");
    var restrictedEndDate = $("#edit-dialog input[data-field-name='RestrictedEndDate']");
    var totalDaysRestricted = $("#edit-dialog input[data-field-name='TotalDaysRestricted']");
    var totalDays = 0;
    //#26B3F7
    if ($("#LostTimeModuleSection").length === 0) {
        $("#edit-dialog input[data-field-name='WaitPeriod']").parent().after($(lostTimeSection));

        $("#LostTimeModuleSection").append("<a href='#' id='LostTimeLogButton' class='losttime-button' aria-claim-id='" + claimID + "' onclick='javascript:ShowLostTimeDialog(this);'>View Lost/Res Time</a>");
        $("#LostTimeModuleSection").append($(lastDayWorkedDate).parent());
        $("#LostTimeModuleSection").append($(lostTimeBeginDate).parent());
        $("#LostTimeModuleSection").append($(lostTimeEndDate).parent());
        $("#LostTimeModuleSection").append($(totalDaysLost).parent());
        $("#LostTimeModuleSection").append($(restrictedBeginDate).parent());
        $("#LostTimeModuleSection").append($(restrictedEndDate).parent());
        $("#LostTimeModuleSection").append($(totalDaysRestricted).parent());

        $(totalDaysLost).css("width", "25px");
        $(totalDaysRestricted).css("width", "25px");
    }
    else {
        $("#LostTimeModuleSection").show();
        $("#LostTimeLogButton").attr("aria-claims-id", claimID);
        $("#LostTimeSectionHeader").show();
    }

    $(lastDayWorkedDate).parent().show();
    $(lostTimeBeginDate).parent().show();
    $(lostTimeEndDate).parent().show();
    $(totalDaysLost).parent().show();
    $(restrictedBeginDate).parent().show();
    $(restrictedEndDate).parent().show();
    $(totalDaysRestricted).parent().show();

    $(lostTimeBeginDate).change(function () {
        if ($(this).val().length > 0) {
           // $(restrictedBeginDate).prop("disabled", true);
           // $(restrictedEndDate).prop("disabled", true);
            $(totalDaysRestricted).prop("disabled", true);
            $(totalDaysLost).prop("disabled", true);
            //$(restrictedBeginDate).empty();
           // $(restrictedEndDate).empty();
           // $(totalDaysRestricted).val("");

            beginDate = new Date($(lostTimeBeginDate).val());
            endDate = new Date($(lostTimeEndDate).val());

            totalDays = (endDate.getTime() - beginDate.getTime()) / (1000 * 3600 * 24);

            $(totalDaysLost).val(Number.isNaN(totalDays) ? "" : totalDays);
        }
        else {
           // $(restrictedBeginDate).prop("disabled", false);
            //$(restrictedEndDate).prop("disabled", false);
            $(totalDaysRestricted).prop("disabled", true);
            $(totalDaysLost).prop("disabled", true);
        }
    });

    $(lostTimeEndDate).change(function () {
        if ($(this).val().length > 0) {
            //$(restrictedBeginDate).prop("disabled", true);
            //$(restrictedEndDate).prop("disabled", true);
            $(totalDaysRestricted).prop("disabled", true);
    ...
    $(restrictedBeginDate).change(function () {
        if ($(this).val().length > 0) {
            //$(lostTimeBeginDate).prop("disabled", true);
           // $(lostTimeEndDate).prop("disabled", true);
            $(totalDaysRestricted).prop("disabled", true);
            $(totalDaysLost).prop("disabled", true);
           // $(lostTimeBeginDate).empty();
           // $(lostTimeEndDate).empty();
            //$(totalDaysLost).val("");

            beginDate = new Date($(restrictedBeginDate).val());
            endDate = new Date($(restrictedEndDate).val());

            totalDays = (endDate.getTime() - beginDate.getTime()) / (1000 * 3600 * 24);

            $(totalDaysRestricted).val(Number.isNaN(totalDays) ? "" : totalDays);
        }
        else {
            //$(lostTimeBeginDate).prop("disabled", false);
            //$(lostTimeEndDate).prop("disabled", false);
            $(totalDaysRestricted).prop("disabled", true);
            $(totalDaysLost).prop("disabled", true);
        }
    });

    $(restrictedEndDate).change(function () {
        if ($(this).val().length > 0) {
            //$(lostTimeBeginDate).prop("disabled", true);
            //$(lostTimeEndDate).prop("disabled", true);
            $(totalDaysRestricted).prop("disabled", true);
            ...
            //$(lostTimeBeginDate).prop("disabled", false);
            //$(lLostTimeEndDate).prop("disabled", false);
            $(totalDaysRestricted).prop("disabled", true);
            $(totalDaysLost).prop("disabled", true);
        }
    });

    $(totalDaysLost).attr("disabled", "disabled");
    $(totalDaysRestricted).attr("disabled", "disabled");
    ...
    function LoadComments() {
        ...
        try {
            $.ajax({
                ...
                error: function (xhr, textStatus, error) { alert("Save Comments Error: " + xhr.responseText); }
            });

        }
        catch (e) {
            alert("Load Comments error: "+e);
        }
    }
    ...
    function LoadDiaryNotes(noteType) {
        ...
        $.ajax({
            ...
            error: function (xhr, textStatus, error) { alert("Load Diary Notes Error: " + xhr.responseText); }
        });
    ...
    function LoadCRComments(showMsg) {
        ...
        if (typeof $(crAnchor).attr("rel") !== 'undefined') {
            ...
            try {
                ...
                alert("Load Comments error: "+e.message);
            }
        }
    }
...
function SaveAllChanges(ele) {
    CloseAccordions();
    ZeroOutOutstandingAmounts();
    SaveReserveChanges();
    RunQuery(ele);
}

function SetAddClaimElements() {

    //Set default InsuredName to BG.
    if (userType === "ClientEditor" && $("#ContentPlaceHolder1_SearchFilter1_txtuserlevel").val().toLowerCase()==="bge") {
        let insuredNameItem = {
            item: { value: "Benton Georgia" }};
        var insuredNameField = $("#edit-dialog input[data-field-name='InsuredName']");
        $(insuredNameField).val("Benton Georgia");
        $(insuredNameField).autocomplete("search","Benton Georgia");
        $(insuredNameField).trigger("autocompleteselect", insuredNameItem);
        $(insuredNameField).click();
        $(insuredNameField).prop("disabled", true);
    }
}
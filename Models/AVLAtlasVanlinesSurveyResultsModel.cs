...
namespace Models
{
    ...
    public class AVLAtlasVanlinesSurveyResultsModel
    {
        #region Properties
        [DataMember]
        ...
        [DataMember]
        public string DriverProfessionalismScore { get; set; } = String.Empty;

        [DataMember]
        public string DriverAtOriginScore { get; set; } = String.Empty;

        [DataMember]
        public string DriverAtDestScore { get; set; } = String.Empty;

        [DataMember]
        public string OverallScore { get; set; } = String.Empty;

        [DataMember]
        public string DriverOverallScore { get; set; } = String.Empty;

        [DataMember]
        public string UseAgainScore { get; set; } = String.Empty;

        [DataMember]
        public string ContactTransfereeScore { get; set; } = String.Empty;
        ...
        public List<AVLAtlasVanlinesSurveyResultsModel> GetSurveyData()
        {
            List<AVLAtlasVanlinesSurveyResultsModel> results = new List<AVLAtlasVanlinesSurveyResultsModel>();
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["reports_aismgtConnectionString"].ConnectionString))
            {
                ...
                cmd.CommandText = "select * from AVL_AtlasVanlines_SurveyResults where Status='Spoke To' ";

                SqlDataReader dr = cmd.ExecuteReader(CommandBehavior.CloseConnection);

                while (dr.Read())
                {
                    var model = new AVLAtlasVanlinesSurveyResultsModel();
